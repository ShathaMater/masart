...
class CompanyTripsPage extends GetView<HomeController> {
  CompanyTripsPage({super.key, required this.companyNew});
  final CompanyNew companyNew;
  List<NewTrip> trips = [];
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      for (var trip in controller.allTrips) {
        if (trip.companyId == companyNew.id) {
          trips.add(trip);
        }
      }
    ...
    return Scaffold(
      appBar: AppBar(title: Text(companyNew.companyName ?? '')),
      body: GetBuilder(
        init: controller,
        builder: (controller) => trips.isEmpty
            ? const Center(child: Text('لا يوجد اي رحلات للشركة'))
            : GridView.builder(
